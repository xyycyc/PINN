...
- [固定节点温度场 schema v1](docs/temperature_field_schema.md)
...
当 `database/raw/calibration_sweep` 存在时，默认 GUI/CLI 建库流程会自动识别真实 case，并输出到：

```text
database/data_process/<material>_case_temperature_field/
```
...
保持默认值即可识别 `database/raw/calibration_sweep`：

- 跳过内置仿真：开启。
- 实验目录：默认仍显示 `raw/10times`；若标准 calibration_sweep 存在，后端自动切换到真实 case 管线。
- 材料：默认 `metal_matrix`。
- 自动划分：开启。
- 测试集比例：默认 `0.2`。
- 随机种子：默认 `42`。
...
- manifest 会自动选择最新 `train_manifest.json`。
...
- schema v1 固定节点 manifest → `direct_point_field`，输出 10,000 点。
...
- 测试 manifest 自动选择最新 `test_manifest.json`。
...
- 默认继承基础 checkpoint 的设备、预处理、训练模式和权重。
- 新权重写入基础任务对应目录，报告写入 `Incremental/<时间戳>/`。
- 完成后自动写入 `database/rule/trained_rules.csv`。

### 4.7 一键演示

点击一次完成建库、划分、训练和校验。如果发现标准 calibration_sweep case，则使用固定节点真实数据；否则回退到旧仿真/实验 CSV 演示流程。

### 4.8 项目清理

按规则三元组扫描 checkpoint、报告、预测结果和规则登记。清理是不可逆操作，执行前请核对窗口列出的路径。
...
存在 `database/raw/calibration_sweep` 时，上述命令自动进入真实固定节点管线。固定节点数据不能与旧规则网格仿真 CSV 静默合并；此时应保持 `--skip-simulation`，不要同时传 `--external-sim-dir`。
...
| `--experiment-material` | 实验材料标签，默认 `metal_matrix` |
...
| 参数 | 含义 |
| --- | --- |
| `--manifest` | 训练清单；不传时尝试根目录下 `train_manifest.json`/`combined_manifest.json` |
...
| `--training-mode` | `normal / residual_pinn` |
| `--physics-residual-weight` | residual PINN 残差权重，默认 0.1 |
| `--learnable-branch-weights` | 启用可学习分支权重 |
| `--fixed-weight-cnn` | CNN 固定权重 |
| `--fixed-weight-lstm` | LSTM 固定权重 |
| `--fixed-weight-material` | 材料分支固定权重 |
| `--fixed-weight-dimension` | 维度分支固定权重 |
| `--fixed-weight-mode` | 稳/瞬态分支固定权重 |
...
```text
sample_id,node_id,x_m,y_m,temperature_k,target_temperature_k,material_id,interface_side
```
...
正式训练时应增大 `--epochs`；1 epoch 只适合验证链路，不代表工程精度。
...
checkpoint 记录 `point_count`、采样指纹和训练集温度统计。预测或增量训练时，任一项不一致都会明确报错。
...
批处理脚本主要服务旧批量实验；固定节点主流程优先使用六个 CLI 入口或 Window。
...
固定节点 manifest 只能使用 `model_kind=direct_point_field`、`checkpoint_version=2` 的 checkpoint。请确认训练和预测使用同一 `sampling_index.npz`、相同 point count 和相同训练集标准化参数。
