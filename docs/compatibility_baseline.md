...
开头记录交付基线；当前界面以本文后续变更及[窗口说明](../window/README.md)为准。
...
- R5 已批准：保存前数字必须补全且有效；旧坏配置逐字段恢复并在界面提示，保存前不改写原文件。
- R6 已批准：训练页新增默认关闭的 `allow_no_validation`，只放行单数据集缺少验证集的情况，不忽略已有验证集，CLI 参数和默认值保持兼容。
- 手动校验不再因空路径触发 CLI 的自动发现；自动模式保留。
- CLI 小数拒绝非有限值，训练器和固定节点标准化增加相同的有效性检查。
- 清理范围覆盖所有增量后代，并校验报告、预测结果及独立模型目标路径；增量训练和独立保留都不得覆盖已有产物。
- 模型加载明确使用项目已有序列化模式；相对基础模型引用跟随模型目录。
- 新增 `tools/check_environment.py` 检查当前解释器、依赖及 Fortran 数值接口。完整验证结论见[本轮审核记录](audit_2026-09-12.md)。
