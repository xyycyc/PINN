...
可先从项目根运行 `python tools/check_environment.py` 检查解释器、依赖及 Fortran 库。若默认 `python` 指向另一个未安装依赖的环境，请激活项目环境或使用解释器绝对路径；窗口及后台任务使用同一个解释器。
...
「路径设置」只显示「保存共用路径」，不会启动任务；它只保存 `common` 段。顶部「保存表单」保存全部页面，数字留空、格式错误或不是有限值时会切换到出错页并定位，且不部分改写配置；修正后保存会清除原错误提示。即使某个数字暂时不参与运行，保存为下次默认值前也需填写。材料划分表中已输入的比例也必须有效。重新加载配置会保留当前页面、日志、折叠状态和自动滚动偏好。

启动或重载旧配置时，已知字段的 `null`、错误类型和非有限数字会逐字段恢复内置默认值；有效设置和扩展字段保留。界面自动展开日志并显示字段和恢复值，原文件保持原样，检查后点击「保存表单」才写回。配置文件语法损坏时也会明确提示并使用内置默认值。
...
项目清理页的扫描和删除按钮固定在底部，删除预览可以滚动。已有任务执行时禁止删除，任务结束后重新生成清理计划；删除仍需确认。关联检查覆盖所有增量后代；保留增量时会保存继承配置并检查目标重名，遇到报告路径越界或目标冲突时拒绝操作；相似名称的其他模型报告及无法确认归属的共享摘要保留。详情见[审核记录](../docs/audit_2026-09-12.md)。
...
训练页新增「允许无验证集训练」，保存字段为 `train.allow_no_validation`，默认关闭。缺少验证清单的单数据集只有明确勾选后才能启动；训练会使用配置轮数，不执行验证早停或最佳验证模型选择。已有验证集仍会使用，开关不会忽略它；多材料集合仍需要各材料的验证清单。CLI 行为保持兼容。

手动模式下，「校验数据」页的清单不能为空且必须是文件，不会因输入框留空而自动切换数据集。校验任务显示完成表示命令执行结束，是否满足需求应查看日志或生成的需求报告。

增量训练输出使用新文件和独立报告目录；基础权重同名、已有输出或已有时间戳报告都会在训练开始前拦截。
...
`gui_smoke.py` 用 Tk 自身的事件循环模拟选页、按钮、输入和重载。三个窗口尺寸覆盖八个页面；检查路径输入宽度、控件边界和底部按钮可见性。还验证输入报错与定位、失败保存不改配置、成功保存清除错误、旧配置恢复及日志提示、无验证集显式开关、手动校验空路径、预览不启动进程、路径选择、真实 `validate` 子进程、停止、失败状态、运行中禁止清理和重载日志保留。校验使用空记录的临时清单，验证的是命令执行与界面反馈，不能用来判断业务数据是否达到验收要求。
